...
def show_items(self):   # 自身の所有するItemインスタンスの在庫状況を、["番号", "商品名", "金額", "数量"]という列でテーブル形式にして出力します。
    ...
    print(tabulate(table_data, headers=["No.", "Product Name", "Amount", "Quantity"], tablefmt="grid"))    # tabulateモジュールを使ってテーブル形式で結果を出力
...
########## definiendo un nuevo metodo que muestra la tabla luego de las compras ##########

    
def show_items_after_purchase(self, indices_seleccionados, cantidades_compradas):  
    table_data = []
    for stock in _stock(self):
        table_data.append([stock['number'], stock['label']['name'], stock['label']['price'], len(stock['items'])])
        for i, item in enumerate(table_data):
            for indice, cantidad in enumerate(cantidades_compradas):
                if item[0] == indices_seleccionados[indice]:
                    table_data[i] = (item[0], item[1], item[2] ,int(len(stock['items']) - cantidad))
    print(tabulate(table_data, headers=["No.", "Product Name", "Amount", "Quantity"], tablefmt="grid"))
